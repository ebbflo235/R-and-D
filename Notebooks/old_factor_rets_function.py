...
def factor_rets (stocks_ret, num_comps):
    ...
    #eig_vals1,eig_vecs1 = np.linalg.eig(cor_mat)
    ...
    for i in range(num_comps):
        ...
        F = np.matmul(stocks_ret[-1:],portfolio_weights) 
...
for i in range(len(stocks_ret.index)-504):
    eigen_ports.append(factor_rets(stocks_ret.iloc[i:i+504],5))
...
X1 = df_factor_rets.iloc[0:60:]
Y = Y_sub.iloc[0:60,0]
X1.index=Y.index
df = p.concat([Y,X1],axis=1)
df.columns = ['Stock','intercept','F1','F2','F3','F4','F5','F6','F7']
result = sm.ols(formula="Stock~ F1+F2+F3+F4+F5+F6+F7",data=df).fit()
result.summary()
resid = result.resid
