...
def factor_rets (stocks, stocks_ret, num_comps):
    ...
    cor_mat = np.corrcoef(stocks_std.T)
    ...
    for i in range(num_comps):
        ...
        eig_val = eig_pairs[i][0]
        portfolio_weights = (1/np.sqrt(eig_val))*Q
        F1 = np.matmul(stocks_ret,portfolio_weights)
        factor_rets.append(F)
    return factor_rets # tuple is easier to see in variable explorer but little difference

factor_returns = []
# if you manually walk through, make sure you reset because stocks_ret shrinks each time you run
for i in range(len(stocks.index)-504):
    factor_returns.append(factor_rets(stocks.iloc[i:i+504],stocks_ret.iloc[i:i+504],3))

#for walk through
#stocks_ret = stocks_ret.iloc[i:i+504]

df_factor_rets = p.DataFrame(factor_returns)
...
#Y = Y_sub.iloc[:,356]
X1.index=Y.index
df = p.concat([Y,X1],axis=1)
df.columns = ['A','Benchmark']
result = sm.ols(formula="A~ Benchmark",data=df).fit()
result.summary()
resid = result.resid
